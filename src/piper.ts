...
export const generatePiperSpeech = async ({
  model,
  speed,
  sentenceSilence,
  text,
  speaker,
  format,
}: SpeakParams): Promise<ReadableStream> => {
  ...
  const piperProcess = spawn(
    ...
    { stdin: Buffer.from(JSON.stringify({ text, speaker })) }
  );

  let stream = piperProcess.stdout;
